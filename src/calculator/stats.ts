...
export const computeSelfBuffs = (
  ...
  for (const [key, buffData] of entries) {
    ...
    if (character === "Qingxiao" && key === "Mindlock") {
      ...
      // it multiplies against the 100% buff from Heaven's Clarity
      if (resonanceChainsConfig?.SequenceNode3DreamsFadeSwordAbides?.isEnabled) {
        data.specificTalentBuffs["HeavenSReckoningEphemeralTranscendenceDMG:totalTalentModifierSpecialMultiply"] = 0.03 * mindlockStacks;
      }
...
export const calculateFinalStatsFromBuffs = (context: {
  ...
  // Step 4a: Compute AdditionalBase buffs using intermediate stats (self buffs)
