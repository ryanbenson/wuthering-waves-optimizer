<template>
        ...
        <div class="flex gap-2 items-center">
          ...
          <div v-if="hasStacks" class="form-control" @click.stop>
            <label
              ...
              <span class="ml-1 text-sm italic">(Max {{ effectiveBuffData.effectiveMaxStacks }})</span>
            </label>
          </div>
        </div>
        <!-- Debug info to show effective values -->
        <div v-if="false && hasStacks && isEnabled" class="text-xs text-gray-500 mt-2">
          <div>Base stacks: {{ stacks }}</div>
          <div>Base max stacks: {{ maxStacks }}</div>
          <div>Effective max stacks: {{ effectiveBuffData.effectiveMaxStacks }}</div>
          <div>Effective modifiers: {{ effectiveBuffData.effectiveModifiers.length }}</div>
        </div>
      </div>
    </div>
  </template>
  ...
  export default {
    ...
    computed: {
      ...
      stacks: {
        get() {
          const stacks = this.currentCharacter?.buffs?.[this.uniqueKey]?.stacks ?? 0;
          return stacks;
        },
        async set(value) {
          const data = {
