import { carlottaRotationStats, carlottaRotationTest001Damages } from "./data/Carlotta/index";
...
describe("Calculator Rotations", () => {
  ...
  it("should enable basic rotations with action buffs", () => {
    ...
    cy.get(`[data-test-rotation-item-by-name="Untitled Rotation"]`).should("be.visible");
    ...
    cy.get(`[data-test-rotation-action-skill-input="none"]`).should("be.visible");
    cy.get(`[data-test-rotation-action-skill-input="none"]`).select("BasicAttackStage1DMG");
    ...
    // find and change the skill used
    cy.get(`[data-test-rotation-action-skill-input="none"]`).should("be.visible");
    cy.get(`[data-test-rotation-action-skill-input="none"]`).select("FatalFinaleDMG");
    ...
    cy.get(`[data-test-action-buff-value-input="CritRate"]`).should("be.visible");
