...
| **`teamRotation.ts`** | `calcTeamRotationDamage` — evaluates a rotation spanning up to 3 characters. In `"basic"` mode (default) each character gets one shared context via `buildCharacterContext.ts`, batched across all their actions. In `"advanced"` mode, each action can carry an `advancedConfig` (from `rotationAdvancedBuffs.ts`); `applyAdvancedOverrides` merges it into that character's stored build data and builds a fresh context per action, then results are summed back into the same per-character/team totals. `convertRotationActionsForSlot(sourceActions, slot, startOrder)` — converts a single-character rotation's actions (from `characters[id].rotations` or a character preset) into team actions pinned to one slot, regenerating ids and continuing the order sequence; used when importing a rotation into a team slot. `calcRotationDps` — shared DPS-from-duration math, used by both Team Rotations and `CalculatorDamages.vue`. |
| **`characterRotation.ts`** | `calcCharacterRotationDamage(rotation, baseContext, characterId, characters, enemyConfig, inventoryEchoes)` — a single-character rotation's counterpart to `teamRotation.ts`'s advanced mode, without a Basic/Advanced mode toggle: actions with no `advancedConfig` share `baseContext` (the caller's already-built context — Calculator.vue passes its own live, reactively-maintained one, so the common no-override case pays no extra cost over what it did before this module existed) via one batched `calcDamages` call; actions that do have a real override (`hasAdvancedConfigOverrides`) each get their own freshly-rebuilt context via `buildCharacterContext.ts`, exactly like Team Rotation's advanced actions. Results from both paths are merged back into the rotation's original action order. Replaced the old per-attack `excludeTeamBuffs`/`excludeWeaponBuffs` category-exclude flags still used by `attacks.ts`'s `calculateAttackDamage` (see issue #401) — those flags remain in `attacks.ts` for backward compatibility but Character Rotations no longer write them. |
