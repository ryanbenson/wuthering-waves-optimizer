  ...
  function buildDetailedCalculationString(
    ...
    talentParts.forEach((part) => {
      if (part.includes("*")) {
        ...
        while (entryIndex < instanceDamageEntries.length && groupedCount < timesNum) {
          const entry = instanceDamageEntries[entryIndex];
          let entryPercentageValue = parseFloat(entry.percentage.replace("%", ""));
        ...
        if (groupedCount > 0) {
          ...
          detailedParts.push(`<strong>${Math.ceil(avgDamage)}</strong> * ${times}`);
        }
      } else {
