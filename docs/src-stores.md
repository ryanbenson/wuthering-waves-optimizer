...
| **teamRotations** | `teamRotations.js` | **`teams`**: array of `{id, name, characterIds: [3], actions: [{id, slot, order, ...same fields as a single-character RotationAction, advancedConfig?}], duration, enemyConfig, mode: "basic" \| "advanced"}`. `mode` gates whether an action's `advancedConfig` (per-buff-category overrides — self buffs, weapon passives, echo set passives, main echo buff, team buffs, resonance chains) is applied; `advancedConfig` is otherwise ignored. Each team references character ids from the `character` store's `characters` map — it does not duplicate build data. Actions: `createTeam`, `renameTeam`, `deleteTeam`, `setTeamCharacter`, `setTeamDuration`, `setTeamActions`, `setTeamEnemyConfig`, `setTeamMode`, `hardSetState`. |
